...
| Natural sine wave $\sum_n n^{-s}\sin(n\pi x)$ | $\mathrm{Im}\,\mathrm{Li}_s(e^{i\pi x})$ (Clausen; Dirichlet damping) | $\Re s > \tfrac{1}{2}$ | $\{n^{-2\sigma}\}$ | $L^2[0,2]$, $\sigma > \tfrac{1}{2}$ | $\zeta$ (energy $\zeta(2\sigma)$, kernel $\zeta(s+\overline{s'})$) | **Complete** (T1–T5 + §7) |
...
| Zeta magnitude $n^{-1/2}$ (**geometric** damping $r^n$, cf. Dirichlet row above) | $2\,\Re\,\mathrm{Li}_{1/2}(re^{i\theta})$ | $r < 1$ | $\{n^{-1/2}r^n\}$ | $L^2(\mathbb{T})$ | Zeros via Weil | Heuristic |
...
| General Dirichlet $L(s,\chi)$ ($\Psi^\chi_s=\sum_n\chi(n)n^{-s}\sin n\pi x$) | cosine/sine $\sum_n\chi(n)r^{\lvert n\rvert}e^{in\theta}$ (PD only for $\chi_0$) | $\Re s>\tfrac12$ (geom. $r<1$ on $\mathbb T$) | $\{\chi(n)n^{-2\sigma}\}$; energy $L(2\sigma,\chi_0)$ | $L^2[0,2]$, $\sigma>\tfrac12$ | $L(s+\overline{s'},\chi_1\bar\chi_2)$; transfer $=$ Gauss-sum rotation sum | **Core complete** (D1–D5 + §6) |
