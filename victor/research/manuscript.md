...
compendium (S00–S06), fifteen work packages, and seven finding notes (F01–F07). Every claim
...
Asano/Lee–Yang gluing route, closed entirely); and one salvage (the present rewindow of the
density gate, which repairs the killed component by redesigning the phase-space window along
the Berry–Keating wedge and reopens the E-track). None of this decides RH, and the program's
...
- **Pre-registered, two-sided criteria.** Each work package states, before any run, what
  ...
  copies the criteria verbatim and applies them mechanically. Three of the seven findings to
  date landed in their falsifier branch — the branches are real, not decorative.
...
3. **Finite stages can carry exact structure without carrying arithmetic.** The stage
   ...
   block-structural rather than arithmetic (F4/F05). Structure is abundant; arithmetic
   transfer is the hard part, exactly as the walls predict.
...
5. **The gate discipline works in both directions.** Three falsifiers fired (F02, F05
   count-census, F06); one gate passed after a derived redesign (F07); every verdict was
   mechanical against criteria fixed in advance. No result depended on a choice made after
   seeing data.
...
F01–F07 in [findings/](findings/); all numerics reproducible from
